...
def test_the_harness_asserts_the_expected_tool_names(tmp_path: Path):
    ...
    assert len(listing["evidence"]["tools"]) == 44  # 36 base + create_github_issue/create_defect_bead/standardize_github_issue (#185) + dashboard_status/dashboard_restart (#207) + dashboard_serve/dashboard_teardown (mc-lj0sh) + queue_status/costs_summary/worktrees_status (#113/#118/#120) + bead_comment (mc-ilia)
...
def test_the_harness_exercises_the_rollout_gate_rather_than_bypassing_it(tmp_path: Path):
    ...
    assert check["passed"] is True
    assert check["evidence"]["internal_tool_count"] == 44  # 36 base + #185 (3) + #207 (2) + mc-lj0sh (2) + #113/#118/#120 (3)
