...
def test_the_harness_exercises_the_rollout_gate_rather_than_bypassing_it(tmp_path: Path):
    ...
    assert check["passed"] is True
    assert check["evidence"]["internal_tool_count"] == 58  # 36 base + #185 (3) + #207 (2) + mc-lj0sh (2) + #113/#118/#120 (3) + molecule_cancel (mc-x06e) + tracker_rows (#186) + beads_list/beads_show (#245) + bead_close/bead_hold/bead_release (mc-p0wps) + artifact_locate (mc-8q0g4) + work_dispatch_bound (mc-vtru8) + events_list (#116) + briefs_pile_state (#226) + formula_dispatch (#256) + pools_status (#197) + provider_set (#197 write half)
