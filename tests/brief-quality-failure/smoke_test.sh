...
grep -Fq 'For every non-repair rejected brief' "$ROOT/formulas/brief-producer-failure-record.toml"
grep -Fq 'brief.quality_failure' "$ROOT/formulas/brief-producer-failure-record.toml"
grep -Fq 'brief-quality-failure-record-backfill.sh' "$ROOT/formulas/brief-producer-failure-record.toml"
grep -Fq 'trigger = "condition"' "$ROOT/orders/brief-producer-failure-record-on-rejected-pile.toml"
grep -Fq '.pile/.rejected' "$ROOT/orders/brief-producer-failure-record-on-rejected-pile.toml"
grep -Fq 'on = "brief.quality_failure_recorded"' "$ROOT/orders/brief-producer-failure-rollup-on-record.toml"
grep -Fq 'trigger = "condition"' "$ROOT/orders/brief-producer-failure-rollup-on-pile.toml"
