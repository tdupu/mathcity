...
- **Claim provenance** — run [[check-claims]] on the drafted brief. It triages which of the brief's claims are load-bearing (negating them changes §2's recommended verdict) and whether each one's instrument actually entails it, then names the independent instrument for the gaps. Settle the cheap gaps inline before deposit; a claim that is UNSOURCED and guards an irreversible verdict (delete / retire / publish / close-someone-else's) is a self-reject, not a caveat. Its Pass 6 coherence check is mandatory whenever the brief was revised section-wise — that is how §2 and §5 end up recommending incompatible sequences. This is a skill-level self-review step, not a gate: it does not appear in `## Gate Evidence` and adds no `gates.toml` entry.
