...
- **A count or census claim** — "N skills", "532 goroutines", "78 rows",
  "18 beads open" — reads as DIRECT ("I counted them") but is **never DIRECT**:
  it is always a SCOPE gap, and often also TEMPORAL. The number is only as
  true as its enumeration boundary, and the boundary is exactly what a quick
  count gets wrong — top-level vs recursive, one store vs all stores, a stale
  snapshot. Route every count through the SCOPE instrument (enumerate the true
  boundary, *then* measure at it) and clear it only when a **structurally
  different** enumeration at the explicit boundary agrees, reconciled against
  an independent authoritative structure — a table, index, or per-part
  breakdown that must sum to the total. The trap that defeats plain
  re-derivation here: two people counting the same obvious way are the **same**
  instrument, not two, so their agreement is not independence — it is a shared
  scope error surviving intact. This is the failure class the corpus caught in
  no other way.
...
- **v1.1** (2026-08-14) — added the count/census rule to Pass 4: a count is
  never DIRECT, it is always a SCOPE gap, and two people counting the same
  obvious way are one instrument, not two. Closes the two skill-store census
  cases the v1.0 corpus caught in no other way — the SCOPE/TEMPORAL blind spot
  named above. (tdupu/mathcity#25)
  [autogenerated by Claude Opus 4.8 on 2026-08-14]
