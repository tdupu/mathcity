...
- **PP1.13 (PROPOSED — direction, not design) Policy-derivable procedures are encoded, not re-derived.** *Status: PROPOSED. This records a DIRECTION from the pack owner (gt-5cls5j, 2026-09-07); the DESIGN of the encoding — class hierarchy, module layout, MCP surface shape — is NOT adjudicated and this rule does not settle it. Governs nothing until Adopted (PP2.1).* A repeated, policy-determined procedure SHOULD be factored into a common Python utility in `mctl` and exposed through the MCP, and a decidable shape SHOULD be named by a Python class, so agents **call and cite** the encoding instead of re-deriving the same decision from prose every session — the variance between agents reading one paragraph is the defect the encoding removes at its source. This is the missing **mechanical enforcement of PP1.10** (derived answers are not decisions): PP1.10 already *mandates* derivation, but has been effectively unenforceable while it requires every agent to hold the whole policy set in working memory and apply it identically — encoding makes the derivation executable rather than re-performed. Guards, each binding even while PROPOSED: (a) encoding **implements** policy and never amends it — on any disagreement between an encoding and the Adopted rule text, the text wins until amended (PP1.7); encoding is not a licence to change the rule. (b) Encoding a **contested** rule is itself a decision and requires adjudication FIRST — an agent may not freeze its own reading of an ambiguous rule into code. (c) Not everything becomes a tool: a rule that cannot be **mechanically evaluated** must REFUSE loudly (CT13.4), never be approximated in code. Per PP2.1 no check skill or gate may cite this rule; it is Adopted only through `new-policy-policy` + check-zero (PP4.5), and adoption ratifies the DIRECTION, not any particular encoding design.
...
| 2026-09-07 | v1.7: add PP1.13 (PROPOSED) — policy-derivable procedures are encoded as Python utilities/classes in `mctl` exposed via MCP so agents call and cite rather than re-derive; marked PROPOSED because the pack owner gave a DIRECTION not a design. Guards: encoding implements, never amends (PP1.7); a contested rule needs adjudication before encoding; a non-mechanically-evaluable rule refuses loudly (CT13.4), not approximated | the pack owner, verbatim (gt-5cls5j, 2026-09-07): "our policies literally become coded in the mctl so we can just run scripts there and we don't need to rely on agents making the same decision every time … When repeated procedures are going to be done, they should be factored out into a common python utility and then put into the MCP." Recorded as direction; the encoding design is explicitly not adjudicated, so the rule is PROPOSED not Adopted. PP1.13 is the missing mechanical enforcement of the Adopted PP1.10 (derived answers are not decisions), which mandates derivation but is unenforceable while every agent must re-derive from prose |
