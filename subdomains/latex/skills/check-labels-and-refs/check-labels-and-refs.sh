...
REPORT_DIR="${OUTPUT_DIR}${WORK_BEAD_ID:+/$WORK_BEAD_ID}"
mkdir -p "$REPORT_DIR"

ROOT_TEX="$ROOT_TEX" REPORT_DIR="$REPORT_DIR" python3 << 'EOF'
import hashlib
import json
import re
import os
...
root_tex = str(Path(root_tex).resolve())
root_dir = str(Path(root_tex).parent)
contents = {}
source_sha256 = {}
scan_errors = []
queue = [(Path(root_tex), 0)]
seen = set()
while queue:
    ...
    try:
        ...
        scan_errors.append({'file': str(file), 'error': str(error)})
        continue
    contents[str(file)] = content
    source_sha256[str(file)] = hashlib.sha256(data).hexdigest()
    if depth < 2:
        for match in re.finditer(r'\\(?:input|include)\{([^}]+)\}', content):
            name = match.group(1)
            if not name.endswith('.tex'):
                name += '.tex'
            queue.append((Path(root_dir) / name, depth + 1))
files_to_scan = contents
...
for file in sorted(files_to_scan):
    for line_num, line in enumerate(contents[file].splitlines(), 1):
...
for file in sorted(files_to_scan):
    for line_num, line in enumerate(contents[file].splitlines(), 1):
        for pattern, ptype in patterns:
            # Ignore macro arguments, not the surrounding reference prose.
            prose = re.sub(r'\\\w*ref\{[^}]*\}', '', line)
...
verdict = 'INCOMPLETE' if scan_errors else ('PASS' if (not orphan_labels and not orphan_refs and not non_pinpoint) else 'FAIL')
