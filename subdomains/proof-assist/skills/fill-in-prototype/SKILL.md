...
description: Work a prototype's gaps: for each conjectural item in a skeleton or spec, dispatch a prover-level attempt at proof or counterexample; harvest results to scratch and the ledger (doubt run recorded before any promotion); successful items become write-proposition candidates. Use when the user says "fill in the prototype", "attack the skeleton", "work the conjectures". NOT for generating new candidates (find-proposition) or writing results into tex (write-proposition, gated).
---
...
2. Per item, dispatch per [../../PROVERS.md](../../PROVERS.md): mandate BOTH directions — prove
   it and hunt a counterexample; partial results (special cases,
   reductions) are results, labeled as such.
3. Harvest per backend contract into
   ...
   `computational`, or REFUTED with the counterexample), evidence
   paths, depends-on edges.
