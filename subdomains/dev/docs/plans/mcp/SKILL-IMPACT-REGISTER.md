...
| --- | --- | --- | --- | --- | --- | --- |
| `check-briefs` — `skills/check-briefs/SKILL.md` | per-brief `bd show` loop parsing a `^Status:` line that `bd` 1.1.0 no longer emits, so the filter was a silent no-op | `replace-with-mctl` (pilot, pre-existing) | `mctl briefs list --json` once per rig; `decision_state` is the filter | read-only; no trace reported | `tests/mctl-shim-callsite/smoke_test.sh` parts 2, 4 | `gt-*` beads keep a direct `bd` fallback, repaired 2026-08-20 to `bd show --json` (the `^Status:` grep it replaced matched nothing on `bd` 1.1.0); frontmatter fields (`unlock_count`, `deposited_at`, `epic`) are unmodelled in `mctl_core` and still scanned from disk |
...
- **`gt-*` beads are unreachable through `mctl`.** The city-root HQ store is not
  ...
  `MCTL_CONTEXT_UNKNOWN_RIG`. `check-briefs` keeps a direct `bd` fallback — as
  of 2026-08-20 a `bd show --json` batch read that branches on the payload type,
  replacing a `^Status:` grep that matched nothing on `bd` 1.1.0 and left every
  closed or deferred `gt-*` brief unfiltered;
