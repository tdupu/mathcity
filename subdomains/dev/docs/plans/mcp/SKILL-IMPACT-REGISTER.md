...
| `adjudicate-brief` — `skills/adjudicate-brief/SKILL.md` | fork body ran `bd comments add` + `bd close`/`bd defer`, rewrote the brief's `status:` frontmatter in place, rewrote the legacy decisions-track manifest, then hand-slung `build-basic-briefed` and eyeballed an assignee grep | `replace-with-mctl` (verdict + dispatch + every cache artifact, step 2b included) | `mctl briefs adjudicate` / `mctl briefs defer` (`--dry-run` preview first); `mctl work status` + `mctl work dispatch` on approve | fork emits `MCTL-TRACE: <id>` per mutation and repeats them in its one-line summary | `tests/mctl-shim-callsite/smoke_test.sh` parts 4-6; `tests/present-briefs-defer-filter/test_defer_filter.sh` executes step 2b's writer; `tests/artifact-root-scoping/smoke_test.sh` | **step 2b is retired and the exemption is gone** — `mctl` now models the legacy row (`effects.py` cache kind `decisions_track_row`, joined through the migration's own `legacy_n`/`legacy_source`), so the write was relocated to the one writer every caller shares rather than deleted. The exemption rested on "deleting it hands the job to nobody", which held only while `mctl` could not do it, and on the skill being the sole caller, which the dashboard route disproved. `BriefCacheAdapter` still owns the artifact and #38 still owns bulk migration of the tree. `gt-*` verdicts have no `mctl` route and are escalated instead |
...
2. **`adjudicate-brief` on approve now slings `work-briefed`, not
   ...
   formula snippets").
3. **`adjudicate-brief` no longer syncs the legacy decisions-track manifest —
   `mctl` does.** *(Superseded 2026-08-20; the reasoning below was right that
   the sync must not simply be deleted, and wrong that the skill had to be the
   one performing it. The skill is one of four callers of the same act, so a
   write that ran only when the skill ran left the row stale whenever the
   verdict came from the dashboard, the CLI or the MCP. `mctl` now plans it as
   a `decisions_track_row` cache update, and step 2b is struck in the skill.
   Everything after this sentence is the original entry, kept for the record.)*

   The first pass of this slice deleted step 2b as a
...
| ~~`adjudicate-brief` step 2b (legacy decisions-track sync)~~ **CLEARED** — `mctl` models the row now, so the sync moved into `plan_adjudication` / `plan_deferral` instead of being deleted | was: `mctl` models neither the file nor the manifest; deleting it re-opens #18 | done |
