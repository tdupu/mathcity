...
| 6 — Typed MCP server | Merged | `mctl_core/{mcp_server,schemas}.py`, `assets/scripts/mctl_mcp_harness.py`, `tests/mctl/test_mcp_{server,schema_snapshots,client_harness}.py`; commits `8b95a30`, `55b63d3`, `f80e21c`, merged `05c825a`. In-slice amendment (line ~837) and the Q5 resolution note are as-built records. **Serving-commit honesty (#210/#172):** `initialize`/`tools/list` now carry `_meta.mctl.serving.{commit,known,started_at}`, captured once at import by `mctl_core/serving.py`, so a caller can detect a stale-bound server; the deliberate rebind (not hot-reload) is [MCTL-MCP-REBIND-PROCEDURE.md](./MCTL-MCP-REBIND-PROCEDURE.md). |
