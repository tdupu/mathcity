...
This document is stored in the city-side development docs so the parallel agent
can fold it into related work:

```text
<city-root>/mathcity/subdomains/dev/docs/
```
...
- Do not move `.beads` ownership between `~/gt` and `~/repos` in this issue.
...
1. Work on a branch in `<repos-root>/mathcity`.
...
12. Use `pr-pipeline-briefed` directed at `<repos-root>/mathcity` and
    issue #40.
