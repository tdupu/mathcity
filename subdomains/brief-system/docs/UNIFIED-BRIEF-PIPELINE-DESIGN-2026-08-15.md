...
The 2026-08-15 #38 ruling chooses fix A: fail closed toward visibility.
Only confidently terminal statuses are preserved as terminal history:
`adjudicated`, `rescinded`, `auto-dispatched`, `moot`, and `superseded`,
prefix-matched so free-text terminal variants remain terminal. Any other
manifest row with an existing legacy file is migration-visible, including
near-misses and mid-decision free text such as `ready-for-adjudication`,
`needs-revision(...)`, `on-hold-needs-revision`, `briefed`,
`brief-prep-dispatched`, and `approved-slung`.
...
6. Run proof 5 over the migration inventory and require zero preserved
   non-terminal manifest rows:
   `python3 tests/decisions-track-migration/proof5_no_nonterminal_unmapped.py <inventory.jsonl>`.
7. Confirm defer does not resurface before `defer_until`.
8. Confirm a forced bad decision-profile brief rejects and records
   `brief_quality_failure`.
9. Run a lost-bead filter fixture through the same lifecycle.
...
7. Apply the #38 Option A classifier/proof-5 patch.
8. Run pre-merge tests, including proof 5 on live inventory in read-only mode.
9. Open PR.
10. BART merges and verifies live-resolved source path.
11. Run pre-deploy checks.
12. Run live canary tests.
13. Run copy-first migration.
14. Run post-migration policy checks.
15. Track rig-local `.beads/briefs` residue as a separate follow-up with its
    own inventory, copy-first migration, and scope proof.
16. Remove legacy fallback in a later, separate change after successful live
    operation.
...
- Proof 5 reports zero non-terminal decisions-track manifest rows preserved
  outside the unified pile/review path before bulk migration.
- Rig-local `.beads/briefs` residue has a separate inventory, copy-first
  migration plan, and scope proof before the system is declared fully unified
  across registered rigs.
