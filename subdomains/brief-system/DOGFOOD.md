...
4. **No-brainer branch** — `no-brainer-classify` formula / `catch-no-brainer` skill copies `no_brainer:true` briefs into `.pile/.no-brainer/`. Auto-execution is a two-position **runtime mode** (2026-08-19): **ARMED by default** per N5 and the owner's ruling — the mode tokens `<city-root>/.beads/no_brainer_auto_execute_armed` and `<rig_root>/.beads/no_brainer_auto_execute_armed` (paths.toml `arm_token_city`/`arm_token_rig`) are brakes, not enablers, so an absent token means auto-execute. DRY-RUN must be pinned by a token reading `false` at either level, or in one command via `brief-check.sh no-brainer-disarm`. Ask which mode is live with `brief-check.sh no-brainer-mode`. The N5 kill switches remain as a second brake. Stop gates G5/G5b fire before either is consulted, and are refused regardless of mode.
...
3. **No-brainer automation is inert.** One brief (`he-h3p2-catch-no-brainer-v0-delivery-brief.md`) has sat in `.pile/.no-brainer/` since early July; the pile-processor (he-x3se) is not shipped; `catch-no-brainer` is still PRELIMINARY (v0.4) and its default mode is now ARMED, and its FP-convergence bead **he-ahfr is explicitly gated on ≥3 dogfood examples**. N6 ("a no-brainer surfacing at the human adjudicator is a regression") cannot be measured because no-brainers never complete the loop.
4. **Mechanical gate validators are not installed.** Formulas reference `mode = "exec"` checks at `.gc/scripts/checks/brief-*.sh`, but `<city-root>/hecke/.gc/` contains only `settings.json` and `tmp/` — no checks directory anywhere in the rig or city `.gc/scripts/`. The pack ships 10 check scripts in `mathcity/assets/scripts/checks/`, but they are not synced to where formulas exec them. Gate enforcement is therefore trust-based frontmatter self-declaration, exactly what he-xkq3 flags as "future: tighten with runnable validators."
   ...
   **RESOLVED 2026-08-19 (drift-audit D9).** The premise was wrong, not just the
   install. There is no supported way to sync `assets/scripts/checks/*` into a
   rig's `.gc/scripts/checks/` — gascity ships no such materialization, and the
   only thing it stages under `.gc/scripts` is a copy of the *city-level*
   `<city-root>/.gc/scripts` directory into agent runtimes
   (`cmd/gc/template_resolve.go`, `citylayout.ScriptsPath`). Hecke's copies are a
   hand-install from the 2026-07-11 pilot and had drifted five weeks stale
   (`brief-check.sh` 9,093 B installed vs 38,801 B shipped). The supported
   mechanism is the `../assets/scripts/checks/<name>.sh` check-path form, which
   gc resolves at cook time to the absolute path of the highest-priority formula
   layer that ships the script (`internal/formula/parser.go`
   `resolveCheckPaths`), and which is a compile error under the v2 graph
   compiler when no layer ships it. All 28 check-path declarations in
   `formulas/` and `gates/` were migrated to that form; no rig install is needed
   or wanted. Verified by execution — see the D9 section of
   `POLICY-DRIFT-AUDIT-2026-08-19.md`.
...
6. Run `catch-no-brainer` on it. Expected: `no_brainer:true, category:cat-A/C, compact_eligible:true`; copy appears in `.pile/.no-brainer/`. **ARMED is the default, so pin DRY-RUN for this step if you do not want it executed — `brief-check.sh no-brainer-disarm`. Confirm with `brief-check.sh no-brainer-mode` before the phase, not by assuming.**
...
| Accidental no-brainer auto-execution during pilot | ARMED is the default, so the pilot is NOT safe by omission — pin DRY-RUN deliberately with `brief-check.sh no-brainer-disarm` (optionally with an `expires=` deadline so it auto-resumes), and verify with `brief-check.sh no-brainer-mode` before each phase rather than assuming. What limits the blast radius if it does fire: stop gates refuse category E regardless of mode, sub-0.85 confidence and missing classifier evidence refuse, and every execution leaves an audit line naming category and confidence |
...
| Formula exec-checks fail hard (missing check script) and block the shuffle mid-pilot | Closed 2026-08-19: check paths now use the `../assets/scripts/checks/…` form, resolved from the pack at cook time, so no rig install can go missing. Residual risk is the opposite one — a check that resolves and runs but reads CWD-relative data (see `nb_registry` in `brief-check.sh`), which refuses rather than permits |
