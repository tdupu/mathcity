...
4. **No-brainer branch** — `no-brainer-classify` formula / `catch-no-brainer` skill copies `no_brainer:true` briefs into `.pile/.no-brainer/`. Auto-execution is a two-position **runtime mode** (2026-08-19): DRY-RUN by default, ARMED only when a valid unexpired token reads `true` at BOTH `<city-root>/.beads/no_brainer_auto_execute_armed` and `<rig_root>/.beads/no_brainer_auto_execute_armed` (paths.toml `arm_token_city`/`arm_token_rig`). Ask with `brief-check.sh no-brainer-mode`; return to DRY-RUN with `brief-check.sh no-brainer-disarm`. The N5 kill switches remain as brakes on top (`auto_merge_enabled`, a file that exists and reads `false` halts). Stop gates G5/G5b fire before either is consulted. **N5 as adopted still says absent-or-`true` proceeds; the arming requirement is a strictly-more-conservative implementation with the amendment DRAFTED, not adopted — `DRAFT-N5-ARMING-AMENDMENT.md`.**
...
3. **No-brainer automation is inert.** One brief (`he-h3p2-catch-no-brainer-v0-delivery-brief.md`) has sat in `.pile/.no-brainer/` since early July; the pile-processor (he-x3se) is not shipped; `catch-no-brainer` is still PRELIMINARY (v0.4) and its mode is DRY-RUN, and its FP-convergence bead **he-ahfr is explicitly gated on ≥3 dogfood examples**. N6 ("a no-brainer surfacing at the human adjudicator is a regression") cannot be measured because no-brainers never complete the loop.
...
6. Run `catch-no-brainer` on it. Expected: `no_brainer:true, category:cat-A/C, compact_eligible:true`; copy appears in `.pile/.no-brainer/`. **Mode stays DRY-RUN — no auto-execute. Confirm with `brief-check.sh no-brainer-mode` before the phase, not by assuming.**
...
| Accidental no-brainer auto-execution during pilot | The mode is DRY-RUN unless someone deliberately armed BOTH levels, so the pilot is safe by default. Verify — do not assume — with `brief-check.sh no-brainer-mode` before each phase, and if it ever reports ARMED, `brief-check.sh no-brainer-disarm` returns it in one command. The N5 kill switch remains available as a second brake |
