...
| revise-return | mathcity | Re-deposit a fresh brief from a revise verdict's instructions (Plan H, #209); re-files to the source rig by default, or a rig the reason names | LANDED, **LIVE E2E PENDING** — six S51 revises are its first real test | gt-5yxup1 FIXED the dead-scan root cause: it scanned the retired/empty `~/.gc/mathcity/briefs`, so all revises re-filed nothing — mctl now mirrors each verdict into the aggregated global root (`~/.gc/mathcity/aggregated-briefs`) and all three brief.decided consumers scan it; #209 (closes only on unattended observation); gascity#32 (lost brief.decided may starve its trigger) |
...
| brief-briefed-base | mathcity | ABSTRACT base owning the shared `file-brief` terminal + `brief-producer.v1` schema ONCE; children `extend` it and override intake/compose-body by id (F8.1). Provenance parameterized via `{{source_formula}}`; `{{terminal_action}}` names the commissioned act | SMOKE 9/9 (`tests/brief-briefed-base`); NOT PROBED live — city down, `gc formula show` unavailable (BART 2026-08-27, ff8d9fe) | Not poured directly (abstract). Two existing siblings NOT yet migrated onto it — see the report-box note below |
| report-fix-briefed | mathcity | Dashboard "Report" box formula (mc-3q4v): rough bug report → evidence-backed fix-brief; APPROVE dispatches build-basic-briefed to fix + PR. `extends brief-briefed-base`; owned-only scope, fail-closed evidence gate | SMOKE 12/12 (`tests/report-fix-briefed`) + evidence gate mutation-proven (`tests/mctl/test_report_fix_evidence_gate.py`, 10/10; vacuous-pass mutant fails 7); NOT PROBED live — brief-operator pool wedged (#10) + city down (BART 2026-08-27, ff8d9fe) | **Stage C (dashboard) BLOCKED — proven P7.3 gap: `work_dispatch` takes only `brief_id` and cannot pin `report-fix-briefed`; needs a typed formula/seed param (Taylor-gated + city-up to validate). Do NOT route around with `gc sling`. No park/hold verb (mc-1pale) → a misrouted dispatch is unrecoverable, so the dispatch path must not ship until pinning is typed** |

> **REPORT-BOX (mc-3q4v) status — BART 2026-08-27, branch `feat/report-box-fix-brief`, UNPUSHED (awaits Taylor's morning authorize-git-operation gate).**
> Landed (ff8d9fe): the formula layer — `brief-briefed-base` + `report-fix-briefed` + the tested evidence gate. Green: 1691 pass / 1 xfail in `tests/mctl`, both formula smoke tests, gate mutation-proven.
> **DEFERRED to a follow-up PR (recorded, not dropped):** migrating the two existing siblings onto the base — `create-issue-briefed` (design Stage A 2nd half) and `pr-pipeline-briefed` (design Stage D). Reason: both create the repo's FIRST multi-level `extends` chain (`brief-briefed-base ← create-issue-briefed ← mathcity-issue-briefed`), which the Python-simulated `mergeSteps` in the smoke tests cannot validate (single-level only) and which `gc formula show` cannot compile-validate while the city is down. Do them once the city is up. The base is built to receive them (provenance parameterized so each sibling stamps its own `source_formula`).
> **DEFERRED — Stage C dashboard surface:** blocked on the proven `work_dispatch` P7.3 gap above (typed formula-pinning) + a dashboard mutation-model decision (the design's `/apply` compound "mint report bead + dispatch" vs. the one-tool-per-operation model). Both are Taylor-gated. `work_claim` is already the typed P1.21 assignee-read the pre-sling check will use.
